...
fs.readdir(path.resolve(__dirname, 'packages'), (err, folders) => {
  ...
  if (err) throw err;
  const readfolders = folders.reduce((acc: any, dir: string) => {
    ...
    return acc.concat(
      new Promise((resolve, reject) => {
        ...
        if (statics) {
          const filesRefs = statics.reduce(
            (acc: any, f: string) => {
              console.log(
                f,
                `js - ${f.endsWith('.js')}`,
              ...
              if (f.endsWith('js'))
                acc.scripts.push({ path: `/javascripts/${dir}/${f}` });
              else if (f.endsWith('css'))
                acc.styles.push({ path: `/javascripts/${dir}/${f}` });
              console.log(acc);
              return acc;
            },
            {
        ...
        fs.mkdir(
          path.resolve(__dirname, `public/javascripts/${dir}`),
          (err) => {
            ...
            fs.readdir(
              path.resolve(__dirname, `packages/${dir}/${configs.publicPath}`),
              (err, files) => {
                if (err) return reject(err);
                const statics = files.reduce(
                  (acc: any, f: string) => {
                    console.log(
                      f,
                      `.../${dir}/${f}`,
  ...
  Promise.all(readfolders)
    .then(() => {
      Object.keys(allConfigs.packages).map((dir) => {
        ncp(
          path.resolve(__dirname, `packages/${dir}/dist`),
          path.resolve(__dirname, `public/javascripts/${dir}`),
          {
            limit: 16,
          },
